<template><div><h1 id="从-版本号打架-到-30-秒内提醒用户刷新-一个微前端团队的实践" tabindex="-1"><a class="header-anchor" href="#从-版本号打架-到-30-秒内提醒用户刷新-一个微前端团队的实践"><span>从&quot;版本号打架&quot;到 30 秒内提醒用户刷新：一个微前端团队的实践</span></a></h1>
<h2 id="_1-背景与痛点" tabindex="-1"><a class="header-anchor" href="#_1-背景与痛点"><span>1. 背景与痛点</span></a></h2>
...
<h3 id="_1-1-真实场景" tabindex="-1"><a class="header-anchor" href="#_1-1-真实场景"><span>1.1 真实场景</span></a></h3>
<ul>
<li>测试在早上打开 dev 页面，下午我们发布了新的组件样式；</li>
<li>他们继续在旧页面里回归，反馈的问题我们一眼看出&quot;这是老版本&quot;；</li>
<li>群里喊&quot;刷新一下&quot;并不靠谱，于是&quot;无效缺陷 + 反复沟通&quot;成了常态。</li>
</ul>
<p>更严重的一次事故，是我们在版本检查逻辑里同时使用了 webpack DefinePlugin 与自定义插件，各自调用了一次 <code v-pre>getAppVersion()</code>。结果前端控制台打印的是 <code v-pre>0.8.3-release-202511210828</code>，而 <code v-pre>version.json</code> 里是 <code v-pre>0.8.3-release-202511210829</code>。两边只差 1 秒钟，却让线上用户始终被提示刷新，形象地被团队称为&quot;版本号打架&quot;。</p>
<h3 id="_1-2-我们的诉求" tabindex="-1"><a class="header-anchor" href="#_1-2-我们的诉求"><span>1.2 我们的诉求</span></a></h3>
<ol>
...
<h2 id="_2-方案探索与取舍" tabindex="-1"><a class="header-anchor" href="#_2-方案探索与取舍"><span>2. 方案探索与取舍</span></a></h2>
<p>在动手前，我们列出几种可行方式：</p>
<table>
<thead>
<tr>
...
<ul>
<li><strong>版本号唯一，可追溯</strong>：<code v-pre>基础版本号-环境-时间戳</code>；</li>
<li><strong>发布零侵入</strong>：Jenkins 仍旧运行 <code v-pre>npm run build-xxx</code>，无需新增步骤。</li>
</ul>
<h2 id="_3-技术方案总览" tabindex="-1"><a class="header-anchor" href="#_3-技术方案总览"><span>3. 技术方案总览</span></a></h2>
<ol>
<li><strong>构建阶段生成 version.json</strong>：在 <code v-pre>vue.config.js</code> 中提前计算版本号，既注入到前端（<code v-pre>process.env.APP_VERSION</code>），也写入输出目录的 <code v-pre>version.json</code>；</li>
<li><strong>前端轮询比对</strong>：应用启动后每 30 秒请求一次 <code v-pre>version.json</code>，禁用缓存并携带时间戳，比较版本号；</li>
<li><strong>交互提示</strong>：复用 Ant Design Vue 的 <code v-pre>Modal.confirm</code>，展示当前/最新版本与环境；</li>
<li><strong>缓存策略</strong>：Nginx 对 HTML/<code v-pre>version.json</code> 禁止缓存，对 JS/CSS/图片继续长缓存；</li>
<li><strong>CI/CD 配合</strong>：推荐在构建命令中传入统一时间戳，确保同一构建任务的所有进程使用相同版本号。</li>
</ol>
<h2 id="_4-关键落地细节" tabindex="-1"><a class="header-anchor" href="#_4-关键落地细节"><span>4. 关键落地细节</span></a></h2>
<h3 id="_4-1-跨平台构建脚本配置-cross-platform-build-script-configuration" tabindex="-1"><a class="header-anchor" href="#_4-1-跨平台构建脚本配置-cross-platform-build-script-configuration"><span>4.1 跨平台构建脚本配置（Cross Platform Build Script Configuration）</span></a></h3>
<p>为了确保 Windows/Linux/macOS 环境都能正确设置时间戳，我们在 <code v-pre>package.json</code> 中使用 <code v-pre>cross-env</code> 和独立的时间戳脚本：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js"><pre v-pre><code><span class="line"><span class="token comment">// scripts/get-timestamp.js</span></span>
<span class="line"><span class="token comment">// 跨平台获取时间戳</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>关键点</strong>：</p>
<ul>
<li><code v-pre>cross-env</code> 确保环境变量在不同操作系统下正确传递</li>
<li>独立的时间戳脚本避免 shell 命令兼容性问题</li>
</ul>
<h3 id="_4-2-版本号只生成一次-build-time-deterministic-versioning" tabindex="-1"><a class="header-anchor" href="#_4-2-版本号只生成一次-build-time-deterministic-versioning"><span>4.2 版本号只生成一次（Build-time Deterministic Versioning）</span></a></h3>
<p>为了彻底解决&quot;版本号打架&quot;问题，我们在 <code v-pre>vue.config.js</code> 中采用了<strong>模块加载时确定时间戳</strong>的策略：</p>
...
<span class="line"><span class="token comment">// ⚠️ 重要：在模块加载时确定构建时间戳，避免多进程构建时版本号不一致</span></span>
<span class="line"><span class="token comment">// Webpack/Vite 多进程构建时，每个 worker 进程独立执行代码，如果多次调用 new Date()</span></span>
<span class="line"><span class="token comment">// 可能在不同进程、不同时间点生成不同的时间戳，导致同一构建产物版本号不一致</span></span>
<span class="line"><span class="token comment">// 方案：优先使用 CI/CD 传入的 BUILD_TIMESTAMP，否则在模块加载时确定统一时间戳</span></span>
...
<span class="line"></span>
<span class="line"><span class="token comment">// 格式化时间戳：年月日时分（如202310301500）</span></span>
...
<span class="line">  <span class="token comment">// 使用模块加载时确定的统一时间戳</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// 获取当前环境名称</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// 生成复合版本号：基础版本+环境+时间戳</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// ⚠️ 关键：构建阶段仅生成一次版本号和环境标识，保持前端注入与 version.json 完全一致</span></span>
...
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">;</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><strong>核心改进点</strong>：</p>
<ol>
<li><strong>优先使用 CI/CD 传入的时间戳</strong>：通过 <code v-pre>BUILD_TIMESTAMP</code> 环境变量，让 Jenkins 统一管理版本号；</li>
<li><strong>模块加载时确定兜底时间戳</strong>：如果没有传入环境变量，在模块加载时（而非函数调用时）确定时间戳，避免多进程构建时产生不同版本号；</li>
<li><strong>全局复用版本号</strong>：<code v-pre>buildVersion</code> 和 <code v-pre>buildEnvName</code> 在模块顶层确定后，被 DefinePlugin 和 version.json 生成逻辑共同使用。</li>
</ol>
<p>这样即使构建过程持续 5 ～ 10 分钟，注入的版本号和静态文件里的版本仍保持一致。这其实是把&quot;构建产物视为不可变工件&quot;的原则落地——保证任何使用该工件的入口看到的元数据都是同一个快照。</p>
<h3 id="_4-3-版本检查器-runtime-polling-cache-busting" tabindex="-1"><a class="header-anchor" href="#_4-3-版本检查器-runtime-polling-cache-busting"><span>4.3 版本检查器（Runtime Polling &amp; Cache Busting）</span></a></h3>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js"><pre v-pre><code><span class="line"><span class="token keyword">class</span> <span class="token class-name">VersionChecker</span> <span class="token punctuation">{</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这里有两个容易被忽略的细节：</p>
<ol>
<li><code v-pre>fetch</code> 显式加 <code v-pre>cache: &quot;no-store&quot;</code>，再叠加时间戳参数，防止 CDN / 浏览器任何一层干预；</li>
<li><code v-pre>visibilitychange</code> 监听，保证窗口重新激活时立即比对，避免用户在后台等了很久才看到弹窗。</li>
</ol>
<p>入口 <code v-pre>main.ts</code> 在应用 mount 之后调用 <code v-pre>versionChecker.init()</code>，即可把整个检测链路串起来。</p>
<h3 id="_4-4-nginx-缓存策略-precise-cache-partition" tabindex="-1"><a class="header-anchor" href="#_4-4-nginx-缓存策略-precise-cache-partition"><span>4.4 Nginx 缓存策略（Precise Cache Partition）</span></a></h3>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这一层的思路是把资源分成两类：需要实时性（HTML、version.json）就 <code v-pre>no-store</code>，其余走长缓存。再配合 <code v-pre>try_files</code> 兜底 history 路由，微前端子应用的独立部署不会互相影响。</p>
<h3 id="_4-5-ci-cd-配置-zero-touch-pipeline" tabindex="-1"><a class="header-anchor" href="#_4-5-ci-cd-配置-zero-touch-pipeline"><span>4.5 CI/CD 配置（Zero-touch Pipeline）</span></a></h3>
<p><strong>推荐方案</strong>：在 Jenkins 构建命令中传入统一的构建时间戳，确保同一构建任务的所有进程使用相同版本号：</p>
<table>
<thead>
<tr>
<th>环境</th>
...
<tr>
<td>develop</td>
<td><code v-pre>BUILD_TIMESTAMP=$(date +%Y%m%d%H%M) npm install &amp;&amp; npm run build-develop</code></td>
<td><code v-pre>npm run build-develop</code></td>
<td>日常开发验证</td>
</tr>
<tr>
<td>testing</td>
<td><code v-pre>BUILD_TIMESTAMP=$(date +%Y%m%d%H%M) npm install &amp;&amp; npm run build-testing</code></td>
<td><code v-pre>npm run build-testing</code></td>
<td>集成测试</td>
</tr>
<tr>
<td>release</td>
<td><code v-pre>BUILD_TIMESTAMP=$(date +%Y%m%d%H%M) npm install &amp;&amp; npm run build-release</code></td>
<td><code v-pre>npm run build-release</code></td>
<td>预发布</td>
</tr>
<tr>
<td>production</td>
<td><code v-pre>BUILD_TIMESTAMP=$(date +%Y%m%d%H%M) npm install &amp;&amp; npm run build-production</code></td>
<td><code v-pre>npm run build-production</code></td>
<td>线上</td>
</tr>
</tbody>
</table>
<p><strong>Windows Jenkins 节点（PowerShell）</strong>：</p>
<div class="language-powershell line-numbers-mode" data-highlighter="prismjs" data-ext="powershell"><pre v-pre><code><span class="line"><span class="token variable">$env</span>:BUILD_TIMESTAMP = <span class="token punctuation">(</span><span class="token function">Get-Date</span> <span class="token operator">-</span>Format <span class="token string">"yyyyMMddHHmm"</span><span class="token punctuation">)</span><span class="token punctuation">;</span> npm install<span class="token punctuation">;</span> npm run build-develop</span>
<span class="line"></span></code></pre>
...
<ul>
<li><strong>推荐方案</strong>：传入 <code v-pre>BUILD_TIMESTAMP</code> 环境变量，由 CI/CD 流水线统一管理时间戳，保证同一构建任务版本号完全一致</li>
<li><strong>简化方案</strong>：不传入时间戳时，代码会在模块加载时确定时间戳，在多进程场景下仍可能出现细微差异（概率较低）</li>
<li><strong>如需精确控制版本号，建议采用推荐方案</strong></li>
</ul>
<p>所有命令都带 <code v-pre>cross-env NODE_OPTIONS=--openssl-legacy-provider</code>，以兼容不同系统的 OpenSSL 版本。更重要的是，这套方案没有&quot;要求运维多做一步&quot;——构建产物天然携带 version.json，任何环境拿到包即可上线。</p>
<h2 id="_5-测试与验证" tabindex="-1"><a class="header-anchor" href="#_5-测试与验证"><span>5. 测试与验证</span></a></h2>
<p>我们定义了一个完整的回归流程，确保方案不会给测试和上线带来额外负担：</p>
<ol>
<li><strong>首次访问</strong>：打开 dev 环境页面，确认控制台打印版本号，Network 里能看到 <code v-pre>version.json</code> 且响应头无缓存；</li>
<li><strong>触发新版本</strong>：调整任意文案，重新发布，保持旧页面不刷新；</li>
<li><strong>轮询验证</strong>：30 秒内弹出提示框，展示当前/最新版本和环境；</li>
<li><strong>交互路径</strong>：
<ul>
<li>点击&quot;立即刷新&quot;：页面强制 reload，新版本生效；</li>
<li>点击&quot;稍后刷新&quot;：记录取消动作并重新开启轮询；</li>
</ul>
</li>
<li><strong>边界场景</strong>：切 tab / 清缓存 / 新设备访问 / 短时间连续发布，均能正确感知最新版本。</li>
</ol>
<h2 id="_6-注意事项与常见问题" tabindex="-1"><a class="header-anchor" href="#_6-注意事项与常见问题"><span>6. 注意事项与常见问题</span></a></h2>
<table>
<thead>
<tr>
<th>现象</th>
...
<tr>
<td>没有弹窗</td>
<td><code v-pre>version.json</code> 404 或版本未变</td>
<td>检查部署路径、确认构建是否生成文件</td>
</tr>
<tr>
<td>弹窗后刷新仍旧版本</td>
<td>静态资源被缓存</td>
<td>核实 Nginx 缓存策略、查看浏览器缓存设置</td>
</tr>
<tr>
<td>构建失败</td>
<td><code v-pre>cross-env</code> 未安装或权限不足</td>
<td>补充依赖、确保 Jenkins 工作目录可写</td>
</tr>
<tr>
<td>持续误报更新</td>
<td>构建阶段多次生成版本号</td>
<td>在 <code v-pre>vue.config.js</code> 顶部缓存 <code v-pre>buildVersion</code> 并全局复用</td>
</tr>
</tbody>
</table>
<h2 id="_7-落地成效" tabindex="-1"><a class="header-anchor" href="#_7-落地成效"><span>7. 落地成效</span></a></h2>
<ul>
...
<h2 id="_8-展望" tabindex="-1"><a class="header-anchor" href="#_8-展望"><span>8. 展望</span></a></h2>
<p>下一步我们计划：</p>
<ol>
...
<hr>
<p>这次实践让我再次意识到：真正的坑往往藏在看似&quot;微不足道&quot;的细节里。当我们把问题和思考写成文档、沉淀成模板，团队就能以更小的代价获得更稳定的交付。如果你也在推进微前端版本同步，欢迎交流、互相借鉴。</p>
</div></template>
